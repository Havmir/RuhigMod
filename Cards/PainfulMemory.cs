            ...
            IModHelper helper) 
    {
        helper.Content.Cards.RegisterCard(new CardConfiguration
        {
            ...
            Name = ModEntry.Instance.AnyLocalizations.Bind(["card", "PainfulMemory", "name"])
                .Localize,
            Art = ModEntry.RegisterSprite(package, "assets/Card/PainfunMemory.png").Sprite
        });
    }
        ...
        if (upgrade == Upgrade.None)
        {
            ...
            if (flipped == false)
            {
                return new CardData()
                {
                    ...
                    floppable = true,
                    artTint = "6868b9",
                    description = "Hull -1 & get a card from your <c=heal>(draw or discard pile)</c> OR exhaust pile"
                };
            }
            if (flipped == true)
            {
                return new CardData()
                {
                    ...
                    floppable = true,
                    artTint = "6868b9",
                    description = "Hull -1 & get a card from your (draw or discard pile) OR <c=heal>exhaust pile</c>"
                };
            }
        }
